...
Look at Fixed effects -> ConditionS -> value in the Pr(>|t|) column is the p value of the "S" contrast, which in our case is Sentence>Word.
...
- under ConditionW we have the __simple effect__ of Condition: Words>Sentences but only for the left hemisphere. And it's indeed pretty small. 
...
In order to estimate the __main effects__, we want to use sum coding. Then, the intercept will reflect the average response for each factor across levels.  
...
Of course, we don't always want to transform everything to sum coding. It might be totally reasonable to estimate the effect of hemispheres for words rather than for the word/sentence average. You just want to be clear about how you're setting your contrasts because it will have a big impact on how to interpret the results.
...
```{r}
m2.bobyqa<-lmer(Effect_Size ~ 1 + Condition + (1 + Condition |SubjectID) + (1 + Condition |Experiment) + (1+Condition |Region), 
                data = data.md.red, 
...
In principle, there is a built in function, `allFit` which will run all the optimizers for you. Then you can look at their fixed effects and determine whether the estimates across all are the same (out to ~4 sig digits) or not. If they are, then it's probably fine to use the model estimates. (Check lme4 documentation for more details) 

### Solution 2: brms 

https://paul-buerkner.github.io/brms/

pro: highly flexible, very similar syntax

con: takes much longer, some new stuff to learn
...
### Solution 4: simplify your model

Simplify your model iteratively, removing random effects which account for least variance first
...
Here that's Condition1 slope by Region so let's try removing that:

```{r}
m2.1 = lmer(Effect_Size ~ 1 + Condition + (1 + Condition|SubjectID) + (1 + Condition|Experiment) + (1 |Region), data = data.md.red)
summary(m2.1)

```

Okay that didn't do it...we could keep going but it's somewhat arbitrary and it's hard to justify throwing out theoretically meaningful slopes
